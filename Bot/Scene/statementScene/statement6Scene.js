...
const fieldKeyboard = [
  ...
  { text: deductionsDate.split(' ')[0], callback_data: deductionsDate },
...
const statement6Scene = new Scenes.BaseScene('statement6Scene');
const columns = 3;

statement6Scene.command('/start', (ctx) => {
  try {
    {
      ctx.deleteMessage();
...
statement6Scene.action(phone, (ctx) => {
  try {
    ctx.reply(
      sharePhone,
...
statement6Scene.on('contact', async (ctx) => {
  try {
...
statement6Scene.action(withSpecialty, (ctx) => {
  try {
...
statement6Scene.action(deductionsDate, (ctx) => {
  try {
...
statement6Scene.action(done, (ctx) => {
  try {
...
statement6Scene.action('yes', async (ctx) => {
  try {
...
statement6Scene.action('statement', (ctx) => {
  try {
    ctx.answerCbQuery();
...
statement6Scene.action('back', (ctx) => {
  try {
    ctx.answerCbQuery();
    return ctx.editMessageText(
      'Заповни всі поля і тоді натисни готово, щоб створити заяву',
...
statement6Scene.action('mainMenu', (ctx) => {
  try {
    ctx.scene.enter('welcomeScene');
  } catch (e) {
    console.log(e);
...
statement6Scene.on('callback_query', (ctx) => {
  ctx.answerCbQuery();
...
statement6Scene.on('message', (ctx) => {
  try {
    ctx.deleteMessage;
  } catch (e) {
    console.log(e);
